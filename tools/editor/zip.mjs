// Portable ZIP (stored entries), no shell tools or additional dependencies.
...
export function zip(entries){
  ...
  for(const [path,value] of entries){
    if(path.startsWith('/')||path.split('/').includes('..'))throw new Error('Caminho inválido no pacote.');
    const name=Buffer.from(path),data=Buffer.isBuffer(value)?value:Buffer.from(value),crc=crc32(data);
